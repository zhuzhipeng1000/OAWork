...
@interface CommonInfoViewController (){
...
@implementation CommonInfoViewController
...
- (void)viewDidLoad {
    ...
    int  imageWith=35;
    NSArray *anArray=@[@{@"title":@"全区通讯录",@"normalImage":@"home_newoa",@"highLightedImage":@"home_newoa"}];
    ...
    _allArray = [@[@{@"title":@"通知公告",@"normalImage":@"home_waitingOA",@"highLightedImage":@"home_waitingOA"},@{@"title":@"查看信息",@"normalImage":@"home_waitingRead",@"highLightedImage":@"home_waitingRead"}] mutableCopy];
...
-(void)bttonTapped:(UIButton*)bt{
    ...
    if (bt.tag==100) {
        ...
        CIMessageListViewController *DCSctrl=[[CIMessageListViewController alloc]init];
        [self.navigationController pushViewController:DCSctrl animated:YES];
        
    }else if(bt.tag==201){
        CIMessageDetailViewController *DCSctrl=[[CIMessageDetailViewController alloc]init];
        [self.navigationController pushViewController:DCSctrl animated:YES];
