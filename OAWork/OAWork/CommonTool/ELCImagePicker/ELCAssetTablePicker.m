...
//        [doneButtonItem setTitle:@"完成"];
//        [self.navigationItem setRightBarButtonItem:doneButtonItem];
        UIButton* lb= [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 40, 30)];
        [lb setTitle:@"确定" forState:UIControlStateNormal];
        [lb setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        UIBarButtonItem* cancelBTN= [[UIBarButtonItem alloc] initWithCustomView:lb];
        [lb addTarget:self action:@selector(doneAction:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    self.columns = CGRectGetWidth(self.view.bounds) / 80;
}
...
- (void)preparePhotos
{
    @autoreleasepool {
        ...
        [self.assetGroup enumerateAssetsUsingBlock:^(ALAsset *result, NSUInteger index, BOOL *stop) {
            ...
            NSString *asstUrl = [[result valueForProperty:ALAssetPropertyAssetURL] description];
            
            for (id value in self.referenceURLInfo) {
                NSString *valueStr;
                if ([value isKindOfClass:[NSDictionary class]]) {
                    valueStr = value[UIImagePickerControllerReferenceURL];
                }
                else if ([value isKindOfClass:[NSString class]]){
                    valueStr = value;
                }
                else continue;
                [elcAsset setSelected:[valueStr isEqualToString:asstUrl]];
                if (elcAsset.selected){
                    [elcAsset setIndex:(int)[self.referenceURLInfo indexOfObject:value]];
//                    [elcAsset setIndex:[[ELCConsole mainConsole] numOfSelectedElements]];
...
{
    [self showLoading];
    [self performSelector:@selector(getPictures) withObject:nil afterDelay:.1];
}
...
-(void)showLoading
{
//    LoadingView* loading= [[LoadingView alloc] initWithLoadingString:@"正在获取图片，请稍等"];
//
    ...
    if ([asset index] < numOfSelectedElements - 1) {
        ...
        for (int i = 0; i < [self.elcAssets count]; i++) {
            ...
            if (assetInArray.selected && ([assetInArray index] > [asset index])) {
                [assetInArray setIndex:[assetInArray index] - 1];
...
- (NSArray *)assetsForIndexPath:(NSIndexPath *)path
{
    ...
    //加入判断防止数组越界
    if((int)self.elcAssets.count >= (index+length)){
